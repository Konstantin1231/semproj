...
true_OU=[-1,1]
true_bi=[-1,1,1]
Drift=np.polynomial.polynomial.Polynomial([0,-1])
Drift_= lambda x: (np.polynomial.polynomial.Polynomial([0,-1]).__call__(x))
'''Invariant measure'''
...
def simulation(delta_time,J,X_0,str_):
    if str_=='OU':
        ...
        for _ in range(J):
            X_old = sim[len(sim) - 1]
            X_new = X_old + -1 * X_old * delta_time + np.sqrt(2 * 1 * delta_time) * norm.rvs()
            sim.append(X_new)
        return sim
...
x=[0,T]
ax1.plot(x,np.ones(len(x))*true_OU[1],color='red',label='True')
ax2.plot(x,np.ones(len(x))*true_OU[0],color='red',label='True')


for delta_time in [0.05,0.01,0.001]:
    J = int(T / delta_time)
    ...
    ax1.plot(xx,sigmas, label=f'h={delta_time}')
    ax2.plot(xx, alphas1, label=f'h={delta_time}')

ax1.set(xlabel='X ', ylabel='$Alpha_1')
ax1.grid()
ax1.legend(loc='center right',prop={'size': 10}, bbox_to_anchor=(1, 0.5))

ax2.set(xlabel='Time(T) ', ylabel='Sigma')
ax2.grid()
ax2.legend(loc='center right',prop={'size': 10}, bbox_to_anchor=(1, 0.5))

plt.savefig(f'OU_different_simulations.jpeg')
