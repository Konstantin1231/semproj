...
for X in [X_2]:
    ...
    J=len(X)
    delta_time=T/J
    xx = delta_time * np.array(range(int(500/delta_time),J,int(500/delta_time)))
    for j in range(int(500/delta_time),J,int(500/delta_time)):
        max_degree = n_lines
    ...
    ax1.plot(xx,sigmas)
    ax2.plot(xx, alphas1, label=f'h={"{:.4f}".format(delta_time)}')

ax1.set(xlabel='X ', ylabel='Sigma')
ax1.grid()
...
plt.savefig(f'OU_same_simulations(3).jpeg')
