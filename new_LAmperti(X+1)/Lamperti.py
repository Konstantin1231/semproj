...
sigma_1=2
X_0=50
lamperti=[(2*alpha_0-2*alpha_1-sigma_1),alpha_1/2,sigma_1]
...
def simulation(delta_time,J,X_0):
    ...
    for _ in range(J):
        X_old=sim[len(sim)-1]
        X_new = X_old + (alpha_1 * X_old +alpha_0)* delta_time + np.sqrt(2*(sigma_1 * (X_old+1))*delta_time)*norm.rvs()
        sim.append(X_new)
    return sim
...
def Matrix_special(X,n_lignes,moments):
    ...
    for i in range(1,n_lignes):
        M[i,:]=np.array([moments[i+1],moments[i],i*(moments[i]+moments[i-1])])
    M[n_lignes,:]=np.array([0,0,(moments[1]+1)])
    return M
...
ax1.grid()
plt.savefig(f'X_0_{X_0}_delta_time_{k*delta_time}_Lamperti_over_time({alpha_1},{alpha_0},{sigma_1},Time={T}).svg')
plt.show()
...
ax1.grid()
plt.savefig(f'X_0_{X_0}_delta_time_{k*delta_time}_quadratique_estimateur_over_time({alpha_1},{alpha_0},{sigma_1},Time={T}).svg')
plt.show()
...
ax1.grid()
plt.savefig(f'X_0_{X_0}_delta_time_{k*delta_time}_Lamperti_error({alpha_1},{alpha_0},{sigma_1},Time={T}).svg')
plt.show()
...
ax1.grid()
plt.savefig(f'X_0_{X_0}_delta_time_{k*delta_time}_compare_quadratique_estimateur_stat({alpha_1},{alpha_0},{sigma_1},Time={T}).svg')
plt.show()
