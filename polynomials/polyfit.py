...
for degree in degres:
    ...
    for P in [np.polynomial.polynomial.Polynomial]:
    ...
    ax1.plot(x, P(parameters[:-1]).__call__(x), label='Degree-'+str(degres[count])+':' + ' ' + f'{"{:.4f}".format(Error(lower,upper,f))}')
    count+=1
...
plt.savefig(f'Bistablepolyfit.jpeg')
